"""
link_recipes_to_target.py

Milestone 2.3 + 2.4:
- Loads recipes (Spoonacular full)
- Loads Target product index (flat)
...
def main():
    ...
    for p in [RECIPES_PATH, TARGET_FLAT_PATH, canonical_path]:
        if not os.path.exists(p):
            raise FileNotFoundError(f"Missing required file: {p}")

    recipes = load_json(RECIPES_PATH)
    target_products = load_json(TARGET_FLAT_PATH)
    canonical_list = load_json(canonical_path)

    id_to_obj, phrase_index = build_canonical_index(canonical_list)

    # 1) Map Target products -> canonical ids (coverage universe)
    target_coverage: Dict[str, List[Dict[str, Any]]] = defaultdict(list)
    unmapped_products = 0

    for prod in target_products:
        cid = map_text_to_canonical(prod.get("name", ""), phrase_index)
        if cid:
            target_coverage[cid].append(prod)
        else:
            unmapped_products += 1
    ...
    for r in recipes:
        ...
        for cid in canonical_ids_unique:
            if cid in target_coverage and len(target_coverage[cid]) > 0:
                covered.append(cid)
            else:
                missing.append(cid)
    ...
    print(f"Target products total: {len(target_products)}")
    print(f"Target products unmapped (expected early): {unmapped_products}")
    ...
    # Show which canonical items are common in recipes but have zero Target matches
    missing_target_for_canonical = []
    for cid, cnt in mapped_recipe_ings.most_common():
        if cid not in target_coverage or len(target_coverage[cid]) == 0:
            missing_target_for_canonical.append((cid, cnt))

    if missing_target_for_canonical:
        print("\nCanonical items used by recipes but missing Target coverage:")
        for cid, cnt in missing_target_for_canonical[:20]:
